...
public class CppCloudiServiceCodegen extends AbstractCppCodegen implements CodegenConfig {
  private final Logger LOGGER = LoggerFactory.getLogger(CppCloudiServiceCodegen.class);

  protected String implFolder = "impl";
  protected String buildFolder = "build";
  protected String externalFolder = "external";
  ...
  public static final String DEFAULT_PACKAGE_NAME = "cpp_cloudi_service";
  ...
  public String getName() {
    return "cpp-cloudi-service";
  }

  @Override
  public String getHelp() {
    return "Generates a C++ API service (based on CloudI)";
  }

  public CppCloudiServiceCodegen() {
    super();

    // TODO: cpp-cloudi-service maintainer review
    modifyFeatureSet(features
        -> features.includeDocumentationFeatures(DocumentationFeature.Readme)
               .securityFeatures(EnumSet.noneOf(SecurityFeature.class))
               .excludeGlobalFeatures(GlobalFeature.XMLStructureDefinitions,
    ...
    apiPackage = "org.openapitools.cloudi.service.api";
    ...
    embeddedTemplateDir = templateDir = "cpp-cloudi-service";

    cliOptions.clear();
    cliOptions.add(new CliOption(
        CodegenConstants.PACKAGE_NAME, "CPP CloudI service name (convention: lowercase).")
  ...
  public void processOpts() {
    ...
    if (additionalProperties.containsKey(CodegenConstants.PACKAGE_NAME)) {
      setPackageName((String) additionalProperties.getOrDefault(
    ...
    supportingFiles.add(
        new SupportingFile("helpers-source.mustache", "model", modelNamePrefix + "Helpers.cpp"));
    supportingFiles.add(new SupportingFile(
        "main-cloudi-service.mustache", "", modelNamePrefix + "main-" + this.packageName + ".cpp"));
    ...
    additionalProperties.put("binFilepath", binFilepath(packageName));
    additionalProperties.put("externalFileFolder", externalFileFolder());
  ...
  public CodegenModel fromModel(String name, Schema model) {
    ...
    if (!codegenModel.isEnum && codegenModel.anyOf.size() > 1
        && codegenModel.anyOf.contains("std::string") && !codegenModel.anyOf.contains("AnyType")
  ...
  public CodegenOperation fromOperation(
    ...
    String pathForCloudi = convertToPathForCloudi(path, httpMethod, true);
    op.vendorExtensions.put("x-codegen-cloudi-path", pathForCloudi);
  ...
  protected String toPrivCloudiFilePath(String name, String extension) {
    return "priv/cloudi/" + name + "." + extension;
  }
  ...
  private String binFilepath(String packageName) {
    return (outputFolder + "/" + buildFolder + "/" + packageName).replace("/", File.separator);
  }

  private String externalFileFolder() {
    return (outputFolder + "/" + externalFolder).replace("/", File.separator);
  }
  ...
  public String convertToPathForCloudi(String path, String method, boolean useMethodSuffix) {
    String serviceName = null;
    String httpMethod = method.toUpperCase(Locale.ROOT);
    String fixedPath = path.replaceAll("\\{(.*?)}", "?");
    if (!useMethodSuffix) {
      serviceName = fixedPath;
    } else if ("GET".equals(httpMethod)) {
      serviceName = fixedPath + "/get";
    } else if ("POST".equals(httpMethod)) {
      serviceName = fixedPath + "/post";
    } else if ("PUT".equals(httpMethod)) {
      serviceName = fixedPath + "/put";
    } else if ("DELETE".equals(httpMethod)) {
      serviceName = fixedPath + "/delete";
    } else if ("HEAD".equals(httpMethod)) {
      serviceName = fixedPath + "/head";
    } else if ("OPTIONS".equals(httpMethod)) {
      serviceName = fixedPath + "/options";
    } else if ("PATCH".equals(httpMethod)) {
      serviceName = fixedPath + "/connect";
    } else if ("TRACE".equals(httpMethod)) {
      serviceName = fixedPath + "/trace";
